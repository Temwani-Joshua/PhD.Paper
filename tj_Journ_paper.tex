...
\usepackage[caption=false,font=footnotesize,farskip=2pt]{subfig}
...
In the latter scenario, high antenna heights and long distances proliferate the dominance of atmospheric scattering and refraction as the characteristic propagation effects. %wherefore radio waves travel a longer distance than the LoS path. Time variability must be considered due to atmospheric variations (changes in the refractive index)
...
\end{equation}
where $L_{fsl}$ is the free space loss derived from the Friis transmission formula   %\footnote{The more accurate term for free space loss is \emph{spreading loss} since it is the result of the inverse square law \cite{Haslett}} %
...
The prediction error, $\varepsilon$\footnote{Not to be confused with permittivity, $\epsilon$}, is the most basic of metrics in evaluating a propagation  model. It is the difference between the measured %\footnote{typically the median value across multiple measurements/predictions}
...
\begin{equation}
\psi = \arctan\left(\dfrac{h_t + h_r}{d_m} \right)
\end{equation}
...
\begin{figure}
	...
	%	\draw (4,0.5) arc (180:220:1cm);
	    ...
	    \draw (1.2,-0.7) node[fill = white, above]{$\epsilon_2=\epsilon{'}$};
...
The FWPM results presented here make use of the reflection coefficient approximation which does not compute the surface wave. However, at the expense of shorter simulation runtime the model can be set to utilize the full Sommerfeld formulation which will incorporate the contribution of the surface wave to the received field. %It must be noted that the simulation runtime doubles when the Sommerfeld equations are . %Surface wave contributions are neglected here as they have been found to be insignificant in the configuration of interest.
%
\subsection{Simulation Setup and Validation}
%Log-Periodic Dipole Antenna (LPDA)  designs were realized in Antenna Magus and subsequently exported to FEKO. A voltage source  was applied at the transmitting antenna so that the far field data could be obtained.
Simulated path loss was computed via S-Parameters as
\begin{equation}
PL_s = 10\log \left|\dfrac{1}{ S_{21}{'}}\right|^2 + 2G %\equiv 10\log\left( \dfrac{\text{power radiated}}{\text{power received}} \right)
,
\end{equation}
where $\left|S_{21}{'}\right|^2 \equiv \text{power received}/\text{power radiated}$ is the transmission coefficient corrected for mismatch and $G$ is the gain of transmitter and receiver in \SI{}{dBi}. %The subscript $s$ denotes simulation.
%
%\subsubsection{Free Space Modelling}
...
Path loss results can change quite dramatically when transmission takes place in the vicinity of the ground such as when transceiver heights are low. %How well a model predicts attenuation depends on the \emph{a priori} information it receives regarding the propagation path and environment. %such as terrain profile.
In a `simple' scenario involving a flat ground, the key issue for the FWPM is with regard to the value of $\dot\epsilon_r$ in modelling the electrical characteristics of the earth. Actual conditions of the soil will seldom be known and some intuitive guesses may be required.\\ %To avoid being subjective, 
...
\begin{table*}%[!ht]
	...
	\begin{tabular*}{\linewidth}{ @{\extracolsep{\fill}} l*{16}c @{}}\toprule %{lcccccccccccccccc}
		$d_m$ & \multicolumn{4}{c}{Mean Prediction Error, $\bar\varepsilon$ [\SI{}{dB}]} & \multicolumn{4}{c}{Maximum Residual, $\varepsilon_{\text{max}}$ [\SI{}{dB}]} & \multicolumn{4}{c}{Standard Dev, $\sigma_{\bar\varepsilon}$ [\SI{}{dB}]} & \multicolumn{4}{c}{RMSE [\SI{}{dB}]}\\ \cmidrule{1-1} \cmidrule{2-5} \cmidrule{6-9} \cmidrule{10-13} \cmidrule{14-17}%\hline\hline
...
%\subsection{Measurements Across Irregular Terrain} % Field Measurements in the Karoo
...
\begin{figure*}
	\centering
	\subfloat[tight]{\includegraphics[width=0.32\linewidth]{hockey-20m}%
	\label{fig:pl20}}%
	\hfil
	\subfloat[tight]{\includegraphics[width=0.32\linewidth]{hockey-50m.pdf}%
	\label{fig:pl50}}%
	\hfil
	\subfloat[tight]{\includegraphics[width=0.32\linewidth]{hockey-100m.pdf}%
	\label{fig:pl100}}
...
\begin{figure*}%[!h]
	\centering
	\subfloat[]{\includegraphics[width=0.32\linewidth]{Karoo_LH_700m}%
	\label{fig:karoo_LH_700m}}%
    \hfil
	\subfloat[]{\includegraphics[width=0.32\linewidth]{Karoo_LH_1800m}%
	\label{fig:karoo_LH_1800m}}%
    \hfil
	\subfloat[]{\includegraphics[width=0.32\linewidth]{Karoo_LH_3600m}%
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{./tj.jpg}}]{Temwani J. Phiri}
Biography text here.
\end{IEEEbiography}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{./Davidson.jpg}}]{David B. Davidson}
Biography text here.
\end{IEEEbiography}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{./Wiid.jpg}}]{P. Gideon Wiid}
Biography text here.
\end{IEEEbiography}
